...
public class EchoServer3ReceiveThread extends Thread {
	...
	public void run() {
		// 연결
		InetSocketAddress remoteAddress = (InetSocketAddress) socket.getRemoteSocketAddress();	//클라이언트 주소받아옴
		...
		try {
			...
			PrintWriter pw = new PrintWriter(new OutputStreamWriter(socket.getOutputStream(), "utf-8"), true); //autoflush - println() 메소드에서는 출력 버퍼를 플래시 한다
